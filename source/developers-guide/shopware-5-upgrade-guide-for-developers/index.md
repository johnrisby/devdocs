...
or warming the full cache. To add these functions to the backend module, you have to extend the constructor to initialize the
`httpCache` property to `themes/Backend/ExtJs/backend/performance/view/main/multi_request_tasks.js` with an additional
entry like this:

```
urlprovidername: {
   providerLabel: '{s name=progress/urlprovidernameLabel}{/s}',
   requestUrl: '{url controller="Performance" action="warmUpCache" resource=urlprovidername}',
}
```
